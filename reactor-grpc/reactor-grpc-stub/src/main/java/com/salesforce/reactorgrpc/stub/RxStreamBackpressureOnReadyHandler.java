...
/**
 * RxStreamBackpressureOnReadyHandler bridges the manual flow control idioms of Reactive streams and gRPC. This class takes
 ...
 * @param <T>
 */
public class RxStreamBackpressureOnReadyHandler<T> implements Subscriber<T>, Runnable {
    private CallStreamObserver<T> requestStream;
    private Subscription subscription;
    private boolean canceled = false;
    private CountDownLatch subscribed = new CountDownLatch(1);

    public RxStreamBackpressureOnReadyHandler(ClientCallStreamObserver<T> requestStream) {
        this.requestStream = Preconditions.checkNotNull(requestStream);
        requestStream.setOnReadyHandler(this);
    }

    public RxStreamBackpressureOnReadyHandler(ServerCallStreamObserver<T> requestStream) {
        this.requestStream = Preconditions.checkNotNull(requestStream);
        requestStream.setOnReadyHandler(this);
