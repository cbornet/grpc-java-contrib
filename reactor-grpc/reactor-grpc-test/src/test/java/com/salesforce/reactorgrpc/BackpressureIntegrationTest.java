...
public class BackpressureIntegrationTest {
    ...
    public void clientToServerBackpressure() throws InterruptedException {
        ...
        Flux<NumberProto.Number> rxRequest = Flux
                .fromIterable(seq)
                .doOnNext(i -> System.out.println(i + " -->"))
                .map(BackpressureIntegrationTest::protoNum);


        Mono<NumberProto.Number> rxResponse = stub.requestPressure(rxRequest);
    ...
    public void serverToClientBackpressure() throws InterruptedException {
        ...
        Mono<Empty> rxRequest = Mono.just(Empty.getDefaultInstance());

        Flux<NumberProto.Number> rxResponse = stub.responsePressure(rxRequest);
        rxResponse.subscribe(
                n -> {
    ...
    public void bidiBackpressure() throws InterruptedException {
        ...
        Flux<NumberProto.Number> rxResponse = stub.twoWayPressure(rxRequest);
